...
public class ArrayDeque<T> extends AbstractDeque<T> {
    ...
    private void resize(int capacity) {
        ...
        //Idea 1 and 2 yield same scores and same failed tests in gradescope
        int i = increment(front, data.length); //Idea #2: change to data.length from size
        //Idea #1: Add below
        /*
        if (i >= capacity * 2) { //try size
            i = 0;
        }
        */
        for (int newIndex = 0; newIndex < size; newIndex += 1) {
            newData[newIndex] = data[i];
            i = increment(i, data.length); //Idea #3: change to data.length from size
        }
